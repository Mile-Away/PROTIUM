...
class Job:
    ...
    def list_by_page(self, job_group_id=0, status=None, startTime=None, endTime=None, page=1, per_page=50):
        ...
        data = self.client.get("/openapi/v1/job/list", params=self.client.params)
        return data
    ...
    def list_by_number(self, number=10, status=None, job_group_id=0):
        ...
        per_page = 50
        job_list = []
        data = self.list_by_page(job_group_id=job_group_id, page=1, per_page=per_page, status=status)
    ...
    def create_job_group(self, project_id, job_group_name):
        data = {"name": job_group_name, "projectId": project_id}
        try:
    ...
    def upload(self, file_path, object_key, token):
        ...
        store = Storage(client=self.client)
        store.upload_From_file_multi_part(object_key=object_key, file_path=file_path, progress_bar=True)
