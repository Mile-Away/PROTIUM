...
class Storage:
    ...
    def write(self, object_key: str = "", data: str = "", parameter: dict = {}, progress_bar: dict = {}) -> dict:

        param = {"path": object_key, "option": parameter}

        if parameter:
            param["option"] = parameter.__dict__
        headers = {}
        headers[self.TIEFBLUE_HEADER_KEY] = self.encode_base64(param)
        req: dict = self.client.post(url="/api/upload/binary", host=self.host, headers=headers, data=data)
        return req

    def read(self, object_key: str = "", token: str = "", ranges: str = "") -> dict:
    ...
    def upload_from_file(
        self, object_key: str = "", file_path: str = "", token: str = "", parameter: Parameter | None = None
    ) -> None:
    ...
    def upload_by_part(self, object_key: str, initial_key: str, chunk_size: int, number: int, body):
        param = {"initialKey": initial_key, "number": number, "partSize": chunk_size, "objectKey": object_key}
        headers = {}
    ...
    def complete_upload_by_part(self, object_key, initial_key, part_string):
        data = {"path": object_key, "initialKey": initial_key, "partString": part_string}
    ...
    def upload_From_file_multi_part(
        ...
        with open(file_path, "r") as f:
            ...
            for c in chunks:
                ...
                part_string.append(
                    self.upload_by_part(
                        object_key, initial_key, chunk_size=c.Size, number=c.Number, body=f.buffer.read(c.Size)
                    ).get("partString")
                )
