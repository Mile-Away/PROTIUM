...
import { AboutSection } from '@/app/dashboard/discussion/[...uuid]/AboutSection';
...
export default function SideInfo({
  ...
  spaceInfo: SpaceProps;
}) {
  return (
    <header
      className="hidden bg-slate-50 backdrop-blur-sm dark:bg-neutral-800/10 lg:fixed lg:inset-y-0 lg:mt-16 lg:flex lg:w-96 
    lg:items-start lg:overflow-y-auto"
    >
      <div
        className="hidden lg:sticky lg:top-0 lg:flex 
      lg:w-16 lg:flex-none lg:items-center lg:whitespace-nowrap 
      lg:py-12 lg:text-sm lg:leading-7 lg:[writing-mode:vertical-rl]"
      >
        <span className="font-mono text-slate-500 dark:text-slate-300">
          Hosted by
        </span>
        <span className="mt-6 flex gap-6 font-bold text-slate-900 dark:text-slate-50">
          {/* {spaceInfo.members.map((host, hostIndex) => (
            <Fragment key={host}>
              {hostIndex !== 0 && (
          ...
          <Fragment>
            <button
              className=" -my-2 flex flex-none items-center gap-2 rounded-xl 
            px-1 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-700"
              onClick={() => {
                window.location.href = `/profile/${spaceInfo.owner}`;
              }}
            >
              {spaceInfo.owner_avatar && (
                <img
                  src={MEDIA_URL + spaceInfo.owner_avatar}
      ...
      <div
        className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4
       lg:min-h-full lg:flex-auto lg:border-x lg:border-slate-200 lg:px-8 lg:py-12 dark:lg:border-neutral-700 xl:px-12"
      >
        <Link
          href="/"
          className="relative mx-auto block w-48 overflow-hidden rounded-lg
          bg-slate-200 shadow-xl shadow-slate-200 dark:bg-neutral-800 dark:shadow-neutral-800
           sm:w-64 sm:rounded-xl lg:w-auto lg:rounded-2xl"
          aria-label="Homepage"
        >
          <img
            className="w-full"
            src={MEDIA_URL + spaceInfo.banner}
            alt=""
            sizes="(min-width: 1024px) 20rem, (min-width: 640px) 16rem, 12rem"
          />
          <div className="absolute inset-0 rounded-lg ring-1 ring-inset ring-black/10 sm:rounded-xl lg:rounded-2xl" />
        </Link>
        <div className="mt-10 flex items-center gap-2 text-left lg:mt-12">
          <img
            src={MEDIA_URL + spaceInfo.icon}
            className="h-6 w-6 rounded-full"
            alt="icon"
          />
          <p className="text-xl font-bold text-slate-900 dark:text-white">
            <Link href="/">{spaceInfo.name}</Link>
          </p>
        </div>
        ...
        <section className="mt-10 lg:mt-12">
          <h2
            className="sr-only flex items-center font-mono text-sm font-medium leading-7
           text-slate-900 dark:text-neutral-50 lg:not-sr-only"
          >
            <TinyWaveFormIcon
              colors={['fill-indigo-300', 'fill-blue-300']}
              className="h-2.5 w-2.5"
            />
            <span className="ml-2.5">Discussions</span>
          </h2>
