'use client';
import { MEDIA_URL } from '@/config';
import { useRouter } from 'next/navigation';

// import { AppContext } from '@/app/providers'
import { Container } from '@/components/Container';
// import { Prose } from '@/components/Prose';
import { formatTime } from '@/lib/formatDate';

import { ArticleProps } from '@/@types/article';
import Markdown from '@/components/docs/Markdwon';
import { useDictCRUD } from '@/hooks/useCrud';
import '@/styles/markdown.css';
import '@/styles/markdown.dark.css';
import '@/styles/markdown.quote.css';
import { useEffect } from 'react';
import ArticleView from '@/components/View';
function ArrowLeftIcon(props: React.ComponentPropsWithoutRef<'svg'>) {
  return (
    <svg viewBox="0 0 16 16" fill="none" aria-hidden="true" {...props}>
      <path
        d="M7.25 11.25 3.75 8m0 0 3.5-3.25M3.75 8h8.5"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}

export default function Page({ params }: { params: { uuid: string[] } }) {
  let router = useRouter();

  const id = params.uuid[0];

  const { dataCRUD, error, isLoading, fetchData } = useDictCRUD<ArticleProps>(
    {} as ArticleProps,
    `/document/article/${id}/`,
  );

  useEffect(() => {
    fetchData();
  }, [id]);

  return (
    <Container className="mt-16 lg:mt-32 ">
      <div className="xl:relative">
        <div className="mx-auto max-w-2xl">
          <button
            type="button"
            onClick={() => router.back()}
            aria-label="Go back to articles"
            className="group mb-8 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-md shadow-zinc-800/5 ring-1 ring-zinc-900/5 transition dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0 dark:ring-white/10 dark:hover:border-zinc-700 dark:hover:ring-white/20 lg:absolute lg:-left-5 lg:-mt-2 lg:mb-0 xl:-top-1.5 xl:left-0 xl:mt-0"
          >
            <ArrowLeftIcon className="h-4 w-4 stroke-zinc-500 transition group-hover:stroke-zinc-700 dark:stroke-zinc-500 dark:group-hover:stroke-zinc-400" />
          </button>

          <article>
            <header className="flex flex-col">
              <h1 className="mb-6 mt-6 text-4xl font-bold tracking-tight text-zinc-800 dark:text-zinc-100 sm:text-5xl">
                {dataCRUD.title}
              </h1>
              <div className="flex items-center justify-between">
                <button
                  name="author"
                  onClick={() => router.push(`/profile/${dataCRUD.author}`)}
                  className="-m-2 flex w-fit items-center gap-x-2  rounded-lg px-2 py-2 text-xs font-semibold text-zinc-600 hover:bg-zinc-50  dark:text-zinc-300 dark:hover:bg-zinc-800/50"
                >
                  <img
                    className="h-5 w-5 rounded-full"
                    src={`${MEDIA_URL}${dataCRUD.avatar}`}
                    alt={dataCRUD.author}
                  />
                  {dataCRUD.author}
                </button>
                {dataCRUD.id && <ArticleView uuid={dataCRUD.uuid} />}
              </div>
              <time
                dateTime={dataCRUD.updated_at}
                className="order-first flex items-center text-base text-zinc-400 dark:text-zinc-500"
              >
                <span className="h-4 w-0.5 rounded-full bg-zinc-200 dark:bg-zinc-500" />
                <span className="ml-3">
                  {formatTime(dataCRUD.updated_at, 'long')}
                </span>
              </time>
            </header>
            {/* <Prose className="mt-8" data-mdx-content> */}
            <div className="markdown">
              <Markdown content={dataCRUD.content} />
            </div>
            {/* </Prose> */}
          </article>
        </div>
      </div>
    </Container>
  );
}
