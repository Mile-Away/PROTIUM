...
<Note>
对于一个 Node 具有自己完整的生命周期（LifeCycle），以所有的 Compile 完成作为生命周期结束的标志。body 内容只在 Node 自己的生命周期内可读。
其它时候只可读到 compile 的结果。
？？Handle 也可以取到上一级的 body source，为什么一定只允许以 Compile 的结果来开放？？
</Note>
...
2. 提取 Node 中的所有 Handle，获取其连接状态，并根据连接状态决定是否需要执行节点中的 Results，
...
3. 当 Node 满足 2.2 中的条件后，获取 Node 中所有需要运行的内容 `Results`，并行执行这个节点中的所有 Results。
    * 当有任意一个 Result 发生错误时，返回 -> `failed`
    * 全部完成时，返回 -> `success`

<Warn>
这个逻辑要求 Node 中的所有 Result 必须全部为可运行的。
</Warn>
